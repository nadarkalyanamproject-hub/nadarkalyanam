...
import { basicDetailsSchema, type BasicDetails } from '@nadar-kalyanam/schemas';
...
type FormState = Record<keyof BasicDetails, string>;
...
export default function BasicDetailsPage() {
  ...
  const [dob, setDob] = useState(() => parseDateOfBirth(data.basicDetails?.dateOfBirth));
  ...
  // `form`/BasicDetails, so it is never saved to the registration draft
  // (no localStorage persistence) and never sent to any API — it is
  // discarded the moment this component unmounts. Revisit only if a future
  // task decides to build real password-based auth.
  ...
  function updateDob(part: 'day' | 'month' | 'year', value: string) {
    ...
    setDob(next);
    update('dateOfBirth', next.day && next.month && next.year ? `${next.year}-${next.month}-${next.day}` : '');
  }
